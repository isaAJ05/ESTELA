...
Por sí mismos, estos estimadores no proporcionan las funciones posteriores necesarias para este proyecto, como la comparación temporal del movimiento, la interpretación de desviaciones o la generación de retroalimentación en lenguaje natural[13], [14]. Por tanto, constituyen un componente de percepción sobre el cual deben construirse las etapas posteriores del sistema.

**Conteo de repeticiones.** La solución `AIGym` de Ultralytics ejecuta estimación de pose y cuenta repeticiones midiendo el ángulo formado por tres puntos articulares conforme un miembro se mueve entre dos umbrales configurables (`up_angle`, `down_angle`), con ejemplos para flexiones, dominadas, sentadillas y extensiones de pierna [16]. Este enfoque evidencia que la estimación de pose puede utilizarse para conteo de repeticiones mediante reglas angulares, pero no aborda directamente la interpretación de desviaciones técnicas ni la generación de retroalimentación contextualizada [16]. El licenciamiento de la librería (AGPL-3.0 para uso de código abierto, según la documentación del proyecto) es además una restricción a considerar en cualquier derivación posterior, y conviene verificarlo directamente en el repositorio antes de asumir compatibilidad.
...
| Criterio | Video pregrabado | Conteo open source (p. ej. [16]) | Comercial con cámara ([20], [23]) | Literatura académica ([6], [7], [9]) | Estela (propuesta) |
| --- | --- | --- | --- | --- | --- |
| Observa la ejecución | No | Sí | Sí | Sí | Sí |
| Interpreta la ejecución respecto a una referencia | No | No | No verificable públicamente | Sí, en algunos trabajos | Sí, objetivo |
...
4. **La extensibilidad constituye una oportunidad arquitectónica.** En varios de los trabajos revisados, la evaluación se desarrolla sobre un conjunto delimitado de ejercicios y utiliza configuraciones específicas para cada uno [7], [8], [9]. Aunque existen conjuntos de datos con un número considerable de actividades [6], [11], estos no corresponden directamente a sistemas de retroalimentación en operación. Esto permite plantear como oportunidad arquitectónica la separación entre el procesamiento general del sistema y la configuración específica de cada ejercicio.

En consecuencia, la revisión sugiere una oportunidad de investigación y desarrollo: evaluar una arquitectura que integre capacidades de estimación de pose, comparación del movimiento e interpretación de la ejecución bajo las restricciones de procesamiento local, retroalimentación en tiempo real y privacidad definidas para el proyecto. A partir de estos antecedentes, el proyecto propone evaluar experimentalmente una arquitectura que integre estimación de pose, comparación del movimiento e interpretación de la ejecución bajo restricciones de procesamiento local y retroalimentación durante la práctica.
