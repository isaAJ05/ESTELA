...
POSE_MODEL = "rtmpose"     # "mediapipe" | "rtmpose"
FEATURE_MODE = "angles_distances"      # "coords" | "angles_distances" | "combined"

RUTA_DATASET = Path(
    rf"C:\Users\USUARIO\Downloads\Prueba2\DATASET\dataset_bilstm\{POSE_MODEL}\{FEATURE_MODE}"
)
...
def main():
    ...
    # --------------------------------------------------------
    # PESOS POR CLASE (el dataset está desbalanceado: p.ej.
    # sentadilla tiene muchos más videos que otras clases)
    # --------------------------------------------------------

    clases_presentes = np.unique(y_train)
    pesos = compute_class_weight(
        class_weight="balanced", classes=clases_presentes, y=y_train
    )
    class_weight = {int(c): float(w) for c, w in zip(clases_presentes, pesos)}

    print("\nPesos por clase (para compensar desbalance):")
    for c, w in class_weight.items():
        print(f"  {clases[c]}: {w:.3f}")
    ...
    ruta_mejor_modelo = RUTA_SALIDA / "modelo.h5"
    ...
    # Guardado explícito del modelo final (además del checkpoint durante
    # el entrenamiento), por si el checkpoint intermedio falla por
    # incompatibilidades de versión de Keras/TensorFlow.
    try:
        modelo.save(str(ruta_mejor_modelo))
    except Exception as e:
        print(f"\n⚠️ No se pudo re-guardar el modelo al final: {e}")
        print("   Si el checkpoint durante el entrenamiento sí se guardó, no hay problema.")
